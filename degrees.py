...
def shortest_path(source, target):
    ...
    """
    # initialise frontier to just starting position IE input actor
    # we will be working with ids
    ...
    while True:
        # if frontier is empty, then we've reached the end of the dataset - no solution
        if frontier.empty():
            return(None)
        ...
        if node.state == target:
            ...
            while node.parent is not None:
                ...
                actions2.append((id_to_movie(node.action), id_to_name(node.state)))
                node=node.parent
            actions.reverse()
            return(actions)
